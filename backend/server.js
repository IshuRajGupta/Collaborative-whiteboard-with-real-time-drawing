...
const allowedOrigin = "https://collaborative-whiteboard-with-real-time-drawing-e0d20j0zt.vercel.app";
app.use(cors({
...
// Add: In-memory canvas history for undo/redo per room
const inMemoryCanvasHistory = new Map(); // roomId -> { stack: [dataUrl], pointer: number }
...
async function getOrCreateRoom(roomId, isPrivate = false, password = '') {
  if (useMongoDB) {
    try {
      ...
      // Always return the found or newly created room, never update isPrivate/password for existing
      return room;
    } catch (error) {
...
function getOrCreateCanvasHistory(roomId) {
  ...
  if (!history) {
    history = { stack: [''], pointer: 0 }; // Start with empty canvas
...
io.on('connection', (socket) => {
  ...
  socket.on('draw', async (data) => {
    ...
    // Save canvas data for persistence and undo/redo
    if (drawData.type === 'end' && drawData.canvasData) {
      const history = getOrCreateCanvasHistory(roomId);
      // If not at top of stack, remove redo states
      ...
      // Persist latest image for reloads
  ...
  socket.on('stroke', async (data) => {
    ...
    // If canvasData is present (for persistence), save it and update history
