...
const Canvas = forwardRef(({ 
  ...
  const currentStroke = useRef([]); // Buffer for current stroke points
  ...
  useImperativeHandle(ref, () => ({
    ...
    loadImageFromDataUrl: (dataUrl) => {
      ...
      img.onload = () => {
        ...
        // Reset undo/redo stacks to match this state
  ...
  const handlePointerDown = (e) => {
    ...
    currentStroke.current = [point]; // Start new stroke buffer
  ...
  const handlePointerMove = (e) => {
    ...
    if (tool === 'pen' || tool === 'eraser') {
      ...
      currentStroke.current.push(point); // Buffer this point
      startPoint.current = point;
  ...
  const handlePointerUp = (e) => {
    ...
    if (tool === 'rectangle' || tool === 'circle') {
      handlePointerMove(e);
      // Emit shape event for collaboration
      if (onDrawEvent) {
        onDrawEvent({
          type: 'shape',
          shape: tool,
    ...
    // Emit full stroke for pen/eraser
    if ((tool === 'pen' || tool === 'eraser') && currentStroke.current.length > 1) {
      ...
      // For other tools, still emit end event for persistence
      onDrawEvent({
        type: 'end',
        canvasData: canvasRef.current.toDataURL('image/png'),
        tool,
  ...
  const handleDoubleClick = (e) => {
    if (tool === 'text') {
      ...
      if (text) {
        ...
        localUndoStack.current.push(ctxRef.current.getImageData(0, 0, CANVAS_WIDTH, CANVAS_HEIGHT));
        localRedoStack.current = [];
        // Emit text event for collaboration
        if (onDrawEvent) {
          onDrawEvent({
            type: 'text',
            text,
